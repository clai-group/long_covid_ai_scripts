...
ccsr_icd <- file.choose() ##the pre-computed delivered with the docker container -- ccsr_pasc_icd.csv
...
ccsr <- data.table::fread(ccsr_icd)
...
df_infect <- merge(df_rmdup, phenxlookup, by ="startPhen", all.x = TRUE)
